		...
		<div class="field-box">
			...
			<el-scrollbar class="right-scrollbar">
				...
				<el-form v-show="currentTab==='field' && showField" size="small" label-width="90px">
					...
					<template v-if="['d2-crud'].indexOf(activeData.__config__.tag) > -1">
						...
						<draggable
							...
							@end="tableRowEnd"
						>
							<div v-for="(item, index) in activeData.columns" :key="index" class="select-item select-Row">
						...
						<el-divider>操作列</el-divider>
						<draggable
							:list="activeData.rowHandle.custom"
							:animation="340"
							group="selectAction"
							handle=".option-drag"
						>
							<div v-for="(item, index) in activeData.rowHandle.custom" :key="index" class="select-item select-action">
								<div class="select-line-icon option-drag">
									<i class="el-icon-s-operation" />
								</div>
								<el-input v-model="item.text" placeholder="标题" size="small" style="margin-right: 4px;" />

								<el-select v-model="item.emit" placeholder="事件类型" size="small" :style="{width: '100%'}">
									<el-option
										v-for="(item, index) in actionCallBackOptions"
										:key="index"
										:label="item.label"
										:value="item.value"
									/>
								</el-select>
								<div class="close-btn select-line-icon" @click="activeData.rowHandle.custom.splice(index, 1)">
									<i class="el-icon-remove-outline" />
								</div>
							</div>
						</draggable>
						...
						<el-form-item label="显示分页">
							<el-switch v-model="activeData.showPagination" @change="changeTablePagination" />
						</el-form-item>
					</template>
					...
					<el-form-item v-if="activeData.__config__.labelWidth!==undefined && activeData.__config__.labelShow" label="标签宽度">
...
export default {
	...
	methods: {
		...
		addRowAction() {
			this.activeData.rowHandle.custom.push({
				icon: 'el-icon-edit',
				text: '',
				type: 'success',
				size: 'small',
		...
		changeTablePagination(val) {
			if (val) {
				...
				this.activeData.pagination = null
			}
		},
		tableRowEnd(val) {
			this.activeData.renderKey = new Date().getTime()
		},
		multipleChange(val) {
