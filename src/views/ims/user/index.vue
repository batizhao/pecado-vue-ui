...
export default {
  ...
  data() {
    ...
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/ims/user/import"
      },
      // 表单校验
      rules: {
        username: [
  ...
  methods: {
    ...
    handleFileSuccess(response, file, fileList) {
      ...
      this.$alert(response.data, '导入成功', { dangerouslyUseHTMLString: true, type: 'success' });
      this.getList();
    },
    // 文件上传失败处理
    handleFileError(err, file, fileList) {
      ...
      this.upload.isUploading = false;
      this.$refs.upload.clearFiles();
      this.$alert((JSON.parse(err.message) || {}).data, '导入失败', { dangerouslyUseHTMLString: true, type: 'error' });
      this.getList();
    },
    // 提交上传文件
    submitFileForm() {
